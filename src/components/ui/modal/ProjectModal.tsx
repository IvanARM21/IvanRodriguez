...
export const ProjectModal = () => {
    ...
    const closeModal = () => {
        ...
        if(!lang) return router.push(pathname, { scroll: false });
                    ...
                    <div className="bg-slate-950 w-full rounded-xl py-6 px-4 sm:p-10 relative flex flex-col items-center lg:flex-row gap-5 overflow-y-auto">
                        ...
                        <div className="flex flex-col max-w-[600px]">
                            <div className="flex-1 flex flex-col gap-5 mb-6">
                                <div>
                                    <h3 className="font-bold text-3xl sm:text-4xl">{project?.title ?? project.description["us"]}</h3>
                                    <p className="sm:text-xl mt-3">{project?.description[lang] ?? project.description["us"]}</p>
                                </div>
                            ...
                            <a href={project.url} className="btn-primary w-fit ml-auto px-4 py-3">
                                {modalLang[lang]?.viewProject ?? modalLang["us"].viewProject}
                            </a>
                        </div>
                    </div>
                )}
